<template>
  ...
  <div idm-ctrl="idm_module" :id="moduleObject.id" :idm-ctrl-id="moduleObject.id">
    ...
    <div class="statistics-card-container">
      <div class="scc-title">
          {{propData.cardTitle||'数量统计'}}
      </div>
      <div class="scc-content">
          <div v-for="(item,index) in statisticsList" :key="index">
            <div class="scc-item-number">{{item.defaultValue}}<span>{{item.numberUnit}}</span></div>
            <div>{{item.title}}</div>
          </div>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    this.statisticsList = this.propData.statisticsList;
  },
  mounted() {
  },
  destroyed() {},
    ...
    setContextValue(object){
      ...
      this.statisticsList.forEach(element => {
        if(object.key==element.dataName){
          ...
          var dataFunction = element.dataFunction;
          ...
          if(_defaultVal!=undefined){
            element.defaultValue = _defaultVal;
          }
        }
      });
    },
